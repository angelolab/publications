# PAH_MIBIPercentPositiveAllMarkersAllRegions.R
# Author: Erin McCaffrey 
# Date created: 200411
# Overview: This script reads in the csv for all PAH cell data and produces data on the percent positive
# for each marker per region.
...
cell_counts<-as.data.frame(table(data$PID, data$region_modified))
colnames(cell_counts)<-c("PID","region_modified","cell_count")
...
for(i in 1:length(markers)) {
  ...
  freq_marker_patient<-as.data.frame(table(data_marker$PID,data_marker$region_modified))
  #append positive counts and percent positive to summary dataframes
  cell_counts[[marker]]<-0
  cell_counts[cell_counts$PID %in% freq_marker_patient$Var1 & cell_counts$region_modified
...
write.csv(cell_counts,"allpatients_count-of-positivecells-perregion.csv",row.names = FALSE)
write.csv(cell_freqs,"allpatients_freq-of-positivecells-perregion.csv",row.names = FALSE)