...
def main():
    ...
    ME_analysis = cell_table[["topic"]]
    count = ME_analysis.groupby(['topic']).size().reset_index()

    # Count number of cells in each ME across all regions
    ...
    plt.savefig(path + '/Sub-panels/SupplementalFigures/SuppFig6c_ME_Distribution_AllCells.png', dpi = 1000, transparent = True)
