...
def main():
    ...
    # Sort cell subsets by medians
    cell_medians = patient_df.drop(columns=["patient"]).median().sort_values(ascending=False)
    cell_order = cell_medians.index.tolist()
    cols = cell_order + ["patient"]
    patient_df_sorted = patient_df[cols]

    # Plot stacked bar plot of cell phenotype proportions for each FOV
    plt.rcParams["figure.figsize"] = (15, 5)
    patient_df_sorted.plot(x='patient', kind='bar', stacked=True, edgecolor='none',
                  title='Proportion of Cells', legend=None, color=[cmap[key] for key in cell_order],# width=0.9,
                  ylim=(0, 1)).get_figure().savefig( path + 
                  "/Sub-panels/SupplementalFigures/SuppFig3a_Proportion_CellPhenotypes_all.pdf", dpi=1000)
    plt.ylim(0, 1)
