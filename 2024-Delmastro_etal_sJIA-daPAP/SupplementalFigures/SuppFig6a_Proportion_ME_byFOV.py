...
def main():
    ...
    # Sort microenvironments by medians
    ME_medians = patient_df.drop(columns=["patient"]).median().sort_values(ascending=False)
    ME_order = ME_medians.index.tolist()
    cols = ME_order + ["patient"]
    ...
    title='Proportion of Microenvironments', legend = None, color = [cmap[key] for key in ME_order], width = 0.9, ylim=(0,1)).get_figure().savefig(path + "/Sub-panels/SupplementalFigures/SuppFig6a_Proportion_ME_all.pdf", dpi = 1000)
    plt.ylim(0,1)
