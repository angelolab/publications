...
# Load data
#--------------------------------
data <- read_csv('data/DBDP_transition/MALDI/library_matched/DBDP_transition_glyRatio_everyGlycan_031725.csv')
...
data_int <- data_filter %>% select(R_H2N2F1:R_H10N9F1) 
...
write.csv(top.table, "data/DBDP_transition/MALDI/library_matched/DE_pval_glycans_DBDP.csv", row.names = F)
